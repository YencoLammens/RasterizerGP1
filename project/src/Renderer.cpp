...
void Renderer::VertexTransformationFunction(const std::vector<Vertex>& vertices_in, std::vector<Vertex>& vertices_out) const
{
	...
	const Matrix view = m_Camera.viewMatrix;
	...
	for (const auto& v : vertices_in)
	{
		...
		out.position = view.TransformPoint(v.position);

		// Perspective divide
		out.position.x /= out.position.z;
		out.position.y /= out.position.z;
		out.position.z = out.position.z;

		// Map to screen space
		out.position.x /= (m_Camera.aspectRatio * m_Camera.fovScale);
		out.position.y /= (m_Camera.fovScale);

		//Map to ndc space
		out.position.x = (out.position.x + 1.f) * 0.5f * m_Width;
		out.position.y = (1.f - (out.position.y + 1.f) * 0.5f) * m_Height;
...
void Renderer::RenderTriangle(const std::vector<Vertex>& vertices) const
{
	...
	const Vector2 edge1 = { v1.x - v0.x, v1.y - v0.y };
	const Vector2 edge2 = { v2.x - v1.x, v2.y - v1.y };
	const Vector2 edge3 = { v0.x - v2.x, v0.y - v2.y };
	...
	for (int py = minY; py <= maxY; ++py)
	{
		...
		{
			//Vector from the vertexes to the pixel
			const Vector2 vertexToPixel1 = { static_cast<float>(px) - v0.x, static_cast<float>(py) - v0.y };
			const Vector2 vertexToPixel2 = { static_cast<float>(px) - v1.x, static_cast<float>(py) - v1.y };
			const Vector2 vertexToPixel3 = { static_cast<float>(px) - v2.x, static_cast<float>(py) - v2.y };

			//Cross between the edges of the triangle and the vectors from vertex to pixel to see if it is inside the triangle
			const float w0 = Vector2::Cross(edge1, vertexToPixel1);
			const float w1 = Vector2::Cross(edge2, vertexToPixel2);
			const float w2 = Vector2::Cross(edge3, vertexToPixel3);
